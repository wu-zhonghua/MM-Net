...
from model.PartNet import PFENet
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    optimizer = torch.optim.SGD(
        [
        {'params': model.down_query_level_2.parameters()},
        {'params': model.down_supp_level_2.parameters()},
        ...
        {'params': model.beta.parameters()},
        {'params': model.part_emb_2_level}],
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and (epoch % 2 == 0 or (args.epochs<=50 and epoch%1==0)):
            loss_val, mIoU_val, mAcc_val, allAcc_val, class_miou = validate(val_loader, model, criterion, epoch)
            if main_process():
...
def train(train_loader, model, optimizer, epoch):
    ...
    sim_loss_2_meter = AverageMeter()
    sim_loss_3_meter = AverageMeter()
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, sim_loss = model(s_x=s_input, s_y=s_mask, x=input, y=target)

        if not args.multiprocessing_distributed:
            main_loss, aux_loss = torch.mean(main_loss), torch.mean(aux_loss)
        loss = main_loss + args.aux_weight * aux_loss + sim_loss
        optimizer.zero_grad()
        ...
        sim_loss_2_meter.update(sim_loss.item(), n)
        # sim_loss_3_meter.update(query_sim_loss.item(), n)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'Sim2Loss {sim_loss_2_meter.val:.4f} '
                        'Sim3Loss {sim_loss_3_meter.val:.4f} '
                        ...
                        'Accuracy {accuracy:.4f}.'.format(epoch+1, args.epochs, i + 1, len(train_loader),
                                                          ...
                                                          sim_loss_2_meter=sim_loss_2_meter,
                                                          sim_loss_3_meter=sim_loss_3_meter,
...
def validate(val_loader, model, criterion, epoch):
    ...
    for e in range(10):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            output_show = output_show.max(1)[1]
